...
public class CertifyHeap {
    ...
    public static boolean isMaxHeap(Comparable[] a) {
        ...
        // For each node 1 <= i <= n / 2, if a[i] is less than either of its children,
        // return
        // false, meaning a[] does not represent a max-heap. If no such i exists, return
        // true.
        for (int i = 1; i <= n / 2; i++) {
            if (less(a[i], a[2 * i]) || 2 * i + 1 <= n && less(a[i], a[2 * i + 1])) {
                return false;
            }
        }
        return true;
